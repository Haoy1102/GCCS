...
def load_data():
    ...
    e1_equal_longtail = pd.read_csv(CSV_E1_EQUAL_LONGTAIL)
    e1_unequal_longtail = pd.read_csv(CSV_E1_UNEQUAL_LONGTAIL)
...
def plot_e1_for_kappa(df, kappa, tag):
    ...
    methods = _ordered_methods(sorted(df_k["method"].unique()))
    ...
    if E1_STYLE == "line":
        ...
        series = {
            m: [df_k[(df_k["method"] == m) & (df_k["rho"] == rho)]["makespan"].values[0]
                for rho in rhos]
            for m in methods
        }
        fig, ax = plt.subplots(figsize=(7.6, 4.4))
    ...
    ax.set_title(f"E1-{tag}: makespan vs rho (kappa={kappa})")
    ax.set_xlabel("rho (workload ratio)")
    ...
    out = OUT_DIR / f"fig_E1_{tag}_kappa{kappa}.png"
    fig.tight_layout()
...
def plot_e2_for_rho(df, rho, tag):
    ...
    ax.set_title(f"E2-{tag}: makespan vs kappa (rho={rho})")
    ax.set_xlabel("kappa (vGPU count per server)")
    ax.set_ylabel(_ylabel("makespan"))
    ...
    out = OUT_DIR / f"fig_E2_{tag}_rho{str(rho).replace('.', '_')}.png"
    fig.tight_layout()
    fig.savefig(out, dpi=200)
    plt.close(fig)
...
def main():
    ...
    all_kappas_equal = sorted(e1_equal["kappa"].unique())
    all_kappas_unequal = sorted(e1_unequal["kappa"].unique())
    all_kappas_equal_longtail = sorted(e1_equal_longtail["kappa"].unique())
    all_kappas_unequal_longtail = sorted(e1_unequal_longtail["kappa"].unique())

    # 为每种数据集生成图表
    for kappa in all_kappas_equal:
        plot_e1_for_kappa(e1_equal, kappa, "equal")
    for kappa in all_kappas_unequal:
        plot_e1_for_kappa(e1_unequal, kappa, "unequal")
    for kappa in all_kappas_equal_longtail:
        plot_e1_for_kappa(e1_equal_longtail, kappa, "equal_longtail")
    for kappa in all_kappas_unequal_longtail:
        plot_e1_for_kappa(e1_unequal_longtail, kappa, "unequal_longtail")
