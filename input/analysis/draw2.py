...
file_path = project_root / "tasks_base_summary.csv"
...
df_cpu = df[df['total_C_TFLOP'] > 0]
axes[0, 0].hist(df_cpu['total_C_TFLOP'], bins=30, alpha=0.7, label='CPU TFLOP', color='blue')
...
df_gpu = df[df['total_G_TFLOP'] > 0]
axes[0, 1].hist(df_gpu['total_G_TFLOP'], bins=30, alpha=0.7, label='GPU TFLOP', color='orange')
...
task_avg_cpu = df_cpu.groupby('template_id')['total_C_TFLOP'].mean()
...
task_avg_gpu = df_gpu.groupby('template_id')['total_G_TFLOP'].mean()
